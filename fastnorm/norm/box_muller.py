...
def _sample_from_ints(ints: Vector[np.uint64]) -> Vector[np.float64]:
    """Sample an array of standard normal 64-bit floats from array of 64-bit unsigned integers."""
    # assumes integer array length is divisible by 2
    z = np.empty(ints.shape[0], dtype=np.float64)
...
def sample(nsamples: int, seed: int) -> Vector[np.float64]:
    ...
    samples = _sample_from_ints(ints)
